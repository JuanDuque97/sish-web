...
import { ServiciosAnalisisCon } from './consultar-idfs.service';
...
import { IGuardarIdfs } from 'src/app/modelo/configuracion/idfs';
...
@Component({
  selector: 'app-consulta-idfs',
  templateUrl: './consultar-idfs.component.html',
})



export class ConsultaIdfsComponent implements OnInit {


  fechaConsulta: number = Date.now();

  public chiEquation: any[] = ['INTENSIDAD=C_1(DURACIÓN+X_0)^C2']
  @ViewChild('pdfTable')
  pdfTable!: ElementRef;
  ...
  public identificaciones = [] as any;
  public correos = [] as any;
  public nombre_solicitantes = [] as any;
  public listObservaciones: any;
  public tipos_cordenada = [] as any;
  ...
  niveles: number;
  public posidcordenadaY = 0;
  public posidcordenadaX = 1;
  imagen: unknown;
  ...
  public ListaTipoCordenadas: any[] = [
    ...
    { id: 3, text: 'EPSG:21897 Bogota 1975 / Colombia Bogota zone' }
  ...
  public listaItemsElementosPintar: any = [];
  ...
  get identificacion() {
    return this.formularioIdfs.get('identificacion')
  }
  get nombre_solicitante() {
    return this.formularioIdfs.get('nombre_solicitante')
  }
  get correo() {
    return this.formularioIdfs.get('correo')
  }
  ...
  agregarElemento() {
    ...
    console.log(this.idtipoCordenada)
    

    if(this.idtipoCordenada == 1){
  
      const cordenadaX = this.formularioConsulta.get('idcordenadaX')?.value;
      const cordenadaY =  this.formularioConsulta.get('idcordenadaY')?.value;
      const pointIn4326 = [cordenadaX, cordenadaY];

     // EPSG:32617 is automatically included in proj4-fully-loaded
     const pointInUTM = proj4("EPSG:4326", "EPSG:21897").forward(pointIn4326);
     var seleccionTemporal: any = {
       ...
       cordenadaX: pointInUTM[0],
       cordenadaY: pointInUTM[1],
     }

     var seleccionTemporalBusqueda: any = {
       id: this.listaItemsElementos.length + 1,
       nombreTipoCordenadas: this.ListaTipoCordenadas.filter(filtro => filtro.id == this.formularioConsulta.get('idtipoCordenadas')?.value)[0].text,
   ...
   if(this.idtipoCordenada == 2){
    ...
    const cordenadaY =  this.formularioConsulta.get('idcordenadaY')?.value;
     ...
     // EPSG:32617 is automatically included in proj4-fully-loaded
     const pointInUTM = proj4("EPSG:3116", "EPSG:21897").forward(pointIn4326);
     var seleccionTemporal: any = {
       id: this.listaItemsElementos.length + 1,
       nombreTipoCordenadas: this.ListaTipoCordenadas.filter(filtro => filtro.id == this.formularioConsulta.get('idtipoCordenadas')?.value)[0].text,
       cordenadaX: pointInUTM[0],
    ...
    var seleccionTemporalBusqueda: any = seleccionTemporal;
      ...
      this.listaItemsElementosPintar.push(seleccionTemporalBusqueda);
      ...
      this.tipos_cordenada.push(seleccionTemporal.nombreTipoCordenadas)
    ...
    Swal.fire({
      ...
      didOpen: async () => {
        ...
        this.ServiciosIdfs.prueba(objetoBusqueda).subscribe((Response) => {
          ...
          if(  this.listObservaciones[0]  == null  ){
            ...
            Swal.fire({
              ...
              timer: 15000, 
              html: 'Por favor ingrese las coordenadas correctamente', 
  ...
  filtrarCalidad2() {
    ...
    let arrayCopyPoz: any = this.datosObservacionList;
    var array: any = {
      idcordenadaX: [], idcordenadaY: []
    }


    for (let index = 0; index < arrayCopyPoz.length; index++) {
      const elementX = arrayCopyPoz[index].idcordenadaX;
      const elementY = arrayCopyPoz[index].idcordenadaY;

      array.idcordenadaX.push(elementY);
      array.idcordenadaY.push(elementX);
    ...
    Swal.fire({
      ...
      didOpen: async () => {
        console.time('executionTime');
      this.ServiciosIdfs.prueba(array)
        .subscribe((Response) => {
          Swal.hideLoading();
  ...
  guadarIdfs() {
    ...
    if (!this.formularioIdfs.valid) {
      this.toast.fire({
        icon: 'error',
        title: 'Los datos son obligatorios y debes ingresar el correo correctamente !!',
      });
    }else{
      Swal.fire({
        title: 'Desea guardar los Idfs?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          ...
          const fechaCreacion:any = Date.now();
          const hoy = new Date(fechaCreacion);
  
          const stringifyObj = JSON.stringify(this.busqueda)
          const b64Str = btoa(stringifyObj)

          let idfs: IGuardarIdfs = {
            consulta:b64Str,
            nombre: this.nombre_solicitantes,
            correo: this.correos,
            identificacion: this.identificaciones
          }
    ...
    cambiarMetodo(even: any) {

      this.idtipoCordenada = 0;
      this.listaItemsElementos = [];
      this.correos = '';
      this.identificaciones = '';
      this.nombre_solicitantes = '';
      this.cordenadaX = 0;
      this.cordenadaY = 0;
      this.formularioConsulta.value.idtipoCordenadas = '';
      this.listaItemsElementosPintar = [];
        Swal.fire({
          title: 'Desea Cambiar el metodo?',
  ...
  seleccionMapa(e: any) {
    ...
    if (this.listaItemsElementos.length < 10) {
      ...
      if(this.idtipoCordenada == 1){
         ...
         const cordenadaX = e.ubicacion.longitude
         const cordenadaY = e.ubicacion.latitude
         const pointIn4326 = [cordenadaX, cordenadaY];
      ...
      if(this.idtipoCordenada == 2){
        ...
        // EPSG:32617 is automatically included in proj4-fully-loaded
        const pointInUTM = proj4("EPSG:4326", "EPSG:3116").forward(pointIn4326);
        var seleccionTemporal: any = {
          id: this.listaItemsElementos.length + 1,
          nombreTipoCordenadas: this.ListaTipoCordenadas.filter(filtro => filtro.id == this.formularioConsulta.get('idtipoCordenadas')?.value)[0].text,
          cordenadaX: pointInUTM[0],
        ...
        const poin = [cordenadaX, cordenadaY];

        // EPSG:32617 is automatically included in proj4-fully-loaded
        const pointI21897 = proj4("EPSG:4326", "EPSG:21897").forward(poin);
        var seleccionTemporalBusqueda: any = {
          id: this.listaItemsElementos.length + 1,
          nombreTipoCordenadas: this.ListaTipoCordenadas.filter(filtro => filtro.id == this.formularioConsulta.get('idtipoCordenadas')?.value)[0].text,
          cordenadaX: pointI21897[0],
          cordenadaY: pointI21897[1],
  ...
  async cargarData(data: string[]) {
    ...
    data.forEach(element => {
      ...
      if (undefined != element && null != element && element.length > 0) {
        ...
        if (null == linea || undefined == linea || linea.length < 1) {
          this.arrayErrores += "--> Error al procesar la línea: " + nLinea + ' con valor: [' + element + ']. Revisar separadores y formatos de los datos. Separador esperado: [' + this.separadorCampos + ']';

        } else {
          const miidcordenadaX = null != linea && undefined != linea && linea.length > 1 ? this.convertirADecimal(linea[this.posidcordenadaX]) : null;
          const miidcordenadaY = null != linea && undefined != linea && linea.length > 1 ? this.convertirADecimal(linea[this.posidcordenadaY]) : null;
          this.totalRegistrosEsperados++;

          if (null != miidcordenadaX && null != miidcordenadaY) {

            let myObject = {
              idcordenadaY: miidcordenadaY,
              idcordenadaX: miidcordenadaX
            };
    ...
    this.datosObservacionList.forEach(observ => {
      console.info("---> Observacion: [coordenadaX: " + observ.idcordenadaX + ", coordenadaY: " + observ.idcordenadaY + "].");
    });
    console.log("-- -------------------------");
  }
  ...
  validarCoordenadas() : boolean {
    let tipoCoordenada :number = +this.formularioConsulta.value.idTipoCoordenadas;
    let longitud :number = +this.formularioConsulta.get('idcordenadaX')?.value
    let latitud :number = +this.formularioConsulta.get('idcordenadaY')?.value
    let norte :number = +this.formularioConsulta.get('idcordenadaX')?.value
    let este :number = +this.formularioConsulta.get('idcordenadaY')?.value
