...
export class ReporteNivelesComponent implements OnInit { 
    fileName= 'Reportes niveles.xlsx';
    datosFilter = [] as any;
    ...
    columnas = [ 
        {title:'NUSPLU',  data:'codigoNesplu',   class: 'text-center'}, 
  ...
  ngOnInit(): void {
    if(this.codigo[0] = 101){
      this.columnas[1]['title'] = "NIVELES (m)";
      this.cargarDatos();
    }
    else if(this.codigo[0] = 101){
      this.toast.fire({
        title: 'Error',
        text: 'No es reporte de Precipitación.',
        icon: 'error'
      });
      return;
    }
 ...
 var datosFilter:any="nesplu,niveles (mm),fecha"

    let titulo = "niveles (mm)"
    let content:any[] = this.datosFilter

    var jsonObject = JSON.stringify(this.datosFilter);

    let headers = ["NESPLU","NIVELES (cm)","FECHA DE MEDICION"]
