...
export class ServiciosObservacionesEmbalsesService {
  ...
  prueba(IObservacionesConsulta: IObservacionesConsulta): Observable<IObservacionesConsulta> { 
    return this.http
      .post<IObservacionesConsulta>(CONFIG.WS_END_POINT + 'observacionesXEmbalseInicial/prueba', IObservacionesConsulta)
      .pipe(
        map((response) => response),
        catchError((e) => {
          HandlerExceptions.validarExcepcionesHTTP(e);
  ...
  cargueRapido(
    IObservacionesEmbalses: IObservacionesEmbalses
  ): Observable<ICargueEmbalseRespuesta> {
    return this.http
      .post<ICargueEmbalseRespuesta>(
        CONFIG.WS_END_POINT + 'observacionesXEmbalseInicial/cargueRapido',
        IObservacionesEmbalses
      )
      .pipe(
        catchError((e) => {
