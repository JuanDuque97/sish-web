...
import { IAforo,  IAforoArchivo,  IAforoCalculo, IAforoDato, IAforoPunto, IAforoView } from 'src/app/modelo/configuracion/aforo';
import { Ihelice } from 'src/app/modelo/configuracion/molinete';
...
@Component({
  selector: 'app-cargar-aforo-nuevo',
  templateUrl: './cargar-aforo-nuevo.component.html', 
})

export class CargarAforoNuevoComponent implements OnInit {
  ...
  public visible = true;
  public nombreArchivoCargueMasivo: any = '';
  public records : any[] = [];
      ...
      this.servicioAforo.obtenerAforo(this.id).subscribe((aforos:any)=>{
        ...
        this.servicioAforo.obtenerAforoDato(this.id).subscribe(datos=>{
          for (let index = 0; index < datos.length; index++) {
           var abscisa =  datos[index].abscisa.toPrecision(2);
           var profundidadTotal =  datos[index].profundidadTotal.toPrecision(2);
           var valorMV =  datos[index].valorMV.toPrecision(2);
           var area =  datos[index].area.toPrecision(2);
           var velocidadMedia =  datos[index].velocidadMedia.toPrecision(2);
           var caudalParcial =  datos[index].caudalParcial.toPrecision(2);
            let objAforo2 = { 
              abscisa:abscisa,
            ...
            var abscisa =  datos[index].abscisa.toPrecision(2);
            var profundidadObservada =  datos[index].profundidadObservada.toPrecision(2);
            var profundidadA =  datos[index].profundidadA.toPrecision(2);
            var revoluciones =  datos[index].revoluciones.toPrecision(2);
            var tiempo =  datos[index].tiempo.toPrecision(2);
            var velocidad =  datos[index].velocidad.toPrecision(2);
  ...
  guardarAforo(){
    if(this.listaDato.length <2){
      ...
      this.servicioAforo.guardarAforoPunto(punto).subscribe(_punto=>{
        ...
        if ( this.assertNullAndUndefined(_punto) && this.assertNullAndUndefined(_punto.idAforoPunto) ) {
          this.listaDato.forEach((item: { abscisa: any; area: any; caudalparcial: any; profundidadTotal: any; VMV: any; velocidadMedia: any; })=>{
            this.listaAbscisa.forEach((dato_: any)=>{
        ...
        /*
        console.log('-- --------------------------------------------------');
        console.log('Mostrando aforoDatos antes de guardar sus valores...');
        console.log('-- --------------------------------------------------');
        this.listaAforoDato.forEach((dato : any) => {
          console.log('--> AforDato: ' + JSON.stringify(dato));
        });
        console.log('-- --------------------------------------------------');
        */
        ...
        this.servicioAforo.guardarAforoDato(this.listaAforoDato).subscribe(aforoDato=>{
            ...
            /*
            console.log('-- --------------------------------------------------');
            console.log('Mostrando CalculoAforo antes de ser guardado...');
            console.log('-- --------------------------------------------------');
            console.log('calculoAforo: ' + JSON.stringify(calculoAforo));
            console.log('-- --------------------------------------------------');
            */
...
cambairDecimal(anny:any){
  ...
  onValidarArchivo(event : any) {
    this.listaAbscisa = [];
    this.listaDato = [];

    if ( !this.assertNullAndUndefined(event) || 
      !this.assertNullAndUndefined(event.target) ) {
        return;
    }

    let target : DataTransfer =  <DataTransfer>(event.target);
    if ( !this.assertNullAndUndefined(target) ) {
        return;
    }

    if ( !this.assertNullAndUndefined(target.files) || target.files.length<=0 ) {
      Swal.fire(
        'Error', 
    ...
    if ( 1 != target.files.length ) {
      Swal.fire(
        'Error', 
        'Solo se puede seleccionar un único archivo', 
        'error',
      );
      return;
    }
    ...
    if ( !this.assertNullAndUndefined(file) || !this.assertNullAndUndefined(file.type) ) {
      Swal.fire(
        'Error', 
        'El archivo solicitado NO es válido', 
        'error'
    ...
    if ( file.type != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" ) {
      Swal.fire(
        'Error', 
        'Solo se permiten archivos de MS Excel.', 
        'error'
      );
      return;
    }

    readXlsxFile(file, {sheet: 'AFO'}).then((rows) => {
      if ( !this.assertNullAndUndefined(rows) ) {
        Swal.fire(
          'Error', 
      ...
      let lastAbsisa : number = 0;
      let lastProfTotal : number = 0;
      let nIndex : number = 0;
      let realIndex : number = -1;
      let realAbsisa : any = -1;
      let realProfTotal : any = -1;
      for ( let i=0;  i<rows.length; i++ ) {
        if ( tableFound && i>=tableIndex ) {
          nIndex++;
        }
        ...
        if ( !this.assertNullAndUndefined(row) || row.length<13 ) {
          continue;
        }

        let profTotalSTR = row[2];
        if ( !tableFound && !this.assertNullAndUndefined(profTotalSTR) ) {
          continue;
        }

        let dataSTR = row[1];

        if ( !tableFound && this.assertNullAndUndefined(dataSTR) && (typeof dataSTR == 'string') && dataSTR.includes('Abscisa') ) {
          tableFound = true;
          tableIndex = i + 2;
        ...
        if ( realIndex>=0 && realIndex<rows.length ) {
          realAbsisa = rows[realIndex][1];
          realProfTotal = rows[realIndex][2];
        }

        if ( !this.assertNullAndUndefined(realAbsisa) || !this.assertNullAndUndefined(realProfTotal) ) {
          continue;
        }

        let dato_absisa = row[1];
        let dato_profTotal = row[2];
        
        let dato_profObserv = row[3];
        let pa = row[4];
        let dato_nRev = row[5];
        let dato_tiempo = row[6];
        let dato_n = row[7];
        let dato_velocidadPromedio = row[8];
        let dato_valorMV = row[9];
        let dato_velocidadMedia = row[10];
        let dato_area = row[11];
        let dato_caudal = row[12];

        if ( !this.assertNullAndUndefined(dato_nRev) ) {
          continue;
        }

        if ( !this.assertNullAndUndefined(dato_profObserv) ) {
          console.error('La profundidad observada: ' + dato_profObserv + ' del registro: ' + (i+1) + ' NO es válido.');
          Swal.fire(
            'Aviso', 
            'El registro de la fila: ' + (i+1) + ' ha sido omitido. El valor de la profundidad observada NO es válido: ' + dato_profObserv, 
            'warning', 
          );
          continue;
        }

        if ( !this.assertNullAndUndefined(dato_tiempo) ) {
          console.error('El tiempo: ' + dato_tiempo + ' del registro: ' + (i+1) + ' NO es válido.');
          Swal.fire(
            'Aviso', 
            'El registro de la fila: ' + (i+1) + ' ha sido omitido. El valor del tiempo NO es válido: ' + dato_tiempo, 
            'warning', 
          );
          continue;
        }

        if ( !this.assertNullAndUndefined(dato_absisa) ) {
          lastAbsisa = realAbsisa;
        }

        if ( !this.assertNullAndUndefined(dato_profTotal) ) {
          lastProfTotal = realProfTotal;
        }

        let registro_absisa = {
          nRow: i+1,
          absisa: !this.assertNullAndUndefined(dato_absisa) ? lastAbsisa : dato_absisa, 
          profTotal: !this.assertNullAndUndefined(dato_profTotal) ? lastProfTotal : dato_profTotal, 
          profObserv: dato_profObserv,
          nRev: dato_nRev, 
          tiempo: dato_tiempo, 
          n: dato_n, 
          velocidadPromedio: dato_velocidadPromedio, 
          valorMV: dato_valorMV, 
          velocidadMedia: dato_velocidadMedia, 
          area: dato_area, 
          caudal: dato_caudal, 
          nDato: nIndex,
        };

        this.records.push(registro_absisa);
      }
      ...
      for ( let i=0; i<this.records.length; i++ ) {
        console.log("--> " + JSON.stringify(this.records[i]));
      }
      console.log("-- -------------");
      */

      this.procesarDatosArchivo();
    });
  }

  procesarDatosArchivo() {
    if ( this.records.length <= 0 ) {
      Swal.fire(
        'Error',
        'Los datos del archivo ingresado NO son válidos.', 
        'error',
      );
      return;
    }
    ...
    this.records.forEach((record : any) => {
      this.abscisa = parseFloat(record.absisa);

      if ( !absisas.includes(this.abscisa) ) {
        absisas.push(this.abscisa);
        this.profundidadTotal = record.profTotal;
        this.calcular();
        this.agregarAbscisaNOValidar();
      }

      this.abscisa = parseFloat(record.absisa);
      this.profundidadTotal = record.profTotal;
      this.profundidadObservada = record.profObserv;
      this.revoluciones = record.nRev;
      this.tiempo = record.tiempo;
      this.calcular();
      this.agregarListaNoValidar();
    });
  }
}
