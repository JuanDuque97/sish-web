...
@Component({
  selector: 'app-calcular-aforo',
  templateUrl: './calcular-aforo.component.html'
  
})

export class CalcularAforoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.id){
      ...
      this.router.navigateByUrl('/configuracion/calcularAforo/E/'+this.id );
  ...
  despintarfechas() {
    ...
    maxFindAndCount(listOfNumbers:any) {
      var result:any = { max: null, count: 0 };
      ...
      try {
        ...
        this.servicioAforo.obtenerAforoDato(this.id).subscribe(datos=>{
          ...
          for (let index = 0; index < datos.length; index++) {
            ...
            var b =  datos[index].profundidadTotal;
          ...
          this.ancho = this.listaCalculo.area/this.radio
          this.perimetroMojado =  this.ancho+2*this.radio   
            ...
            this.velocidadMedia  = this.listaCalculo.caudal / this.listaCalculo.area;
                    var cantidad =[];
                    var cantidadLinea =[];
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              this.servicioAforo
              .actualizarEstadoObservacion(this.observacion,this.id,1,this.listaCalculo.caudal,this.h)
